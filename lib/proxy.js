...
function isProxyDomain(entry) {
  return Boolean(entry && entry.published && entry.mode === "proxy" && entry.target);
}

function getPublishedProxyDomain(host, state) {
  const entry = state.domains[host];
  return isProxyDomain(entry) ? entry : null;
}
...
function createProxyServers(paths = getPaths(), options = {}) {
  ...
  const httpsServer = https.createServer(
    ...
    (req, res) => {
      ...
      const entry = getPublishedProxyDomain(host, loadState(paths));
      if (!entry) {
        res.writeHead(404, { "content-type": "text/plain; charset=utf-8" });
        res.end("No published proxy domain matches this host.\n");
