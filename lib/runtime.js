...
async function startRuntime(paths = getPaths()) {
  ...
  const domainOnlyDomains = domains.filter((entry) => entry.enabled && entry.mode === "domain-only");
  const enabledDomains = domains.filter((entry) => entry.enabled);
  const discovery = startDiscovery(enabledDomains);
  ...
  if (!hasActiveRuntime) {
    return {
      runtime: "idle",
      message: "No enabled domains with valid discovery or proxy settings.",
      proxyDomains: 0,
