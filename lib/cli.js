...
const { CA_HELP_HOST } = require("./proxy");
...
const { applyInstall, applyUninstall } = require("./system");
const { version } = require("../package.json");

function assertSupportedPlatform() {
  if (process.platform !== "darwin") {
    throw new Error("lanx currently supports macOS only.");
  }
}
...
function normalizeName(name) {
  ...
  if (domain === CA_HELP_HOST) {
    throw new Error(`${CA_HELP_HOST} is reserved for lanx internal routes`);
  }
  return domain;
}
...
function formatDomainRow(domain, entry) {
  ...
  return [domain.padEnd(24), String(entry.mode).padEnd(12), endpoint].join(" ");
}
...
async function runCli(argv) {
  ...
  if (command === "start") {
    ...
    const caHelpUrl =
      running.httpsPort === 443
        ? `https://${CA_HELP_HOST}/`
        : `https://${CA_HELP_HOST}:${running.httpsPort}/`;
    console.log(`CA helper: ${caHelpUrl}`);
  ...
  if (command === "add") {
    ...
    if (!name) {
      throw new Error("Usage: lanx add <name> [--port <n> | --target <url>]");
    }
  ...
  if (command === "edit") {
    ...
    const name = args.shift();
    if (!name) {
      throw new Error("Usage: lanx edit <name> [--target <url> | --port <n>]");
    }
    const domain = normalizeName(name);
