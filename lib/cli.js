...
function parseEnabledState(args) {
  const enable = hasFlag(args, "--enable");
  const disable = hasFlag(args, "--disable");
  if (enable && disable) {
    throw new Error("Provide only one of --enable or --disable");
  }
  if (enable) {
    return true;
  }
...
function formatDomainRow(domain, entry) {
  ...
  return [
    ...
    String(entry.enabled).padEnd(10),
    endpoint,
  ].join(" ");
}
...
async function runCli(argv) {
  ...
  if (command === "uninstall") {
    assertOnlyFlags(args, []);
    const state = loadState(paths);
    applyEnabledHosts({ ...state, domains: {} });
  ...
  if (command === "start") {
    ...
    if (running.runtime === "proxy") {
      ...
      console.log(`Enabled proxy domains: ${running.proxyDomains}`);
      console.log(`Enabled domain-only domains: ${running.domainOnlyDomains}`);
  ...
  if (command === "list") {
    const state = loadState(paths);
    console.log("DOMAIN                   MODE         ENABLED    ENDPOINT");
  ...
  if (command === "add") {
    ...
    if (!name) {
      throw new Error(
        "Usage: lanx add <name> [--port <n> | --target <url>] [--enable | --disable]",
      );
    }
    ...
    assertOnlyFlags(args, ["--port", "--target", "--enable", "--disable"]);

    const targetFlag = parseFlag(args, "--target");
    const portFlag = parseFlag(args, "--port");
    const enabledState = parseEnabledState(args);
    ...
    const entry = normalizeDomainEntry(domain, {
      ...
      target,
      protocol: "https",
      port: mode === "proxy" ? 8443 : parsePort(portFlag),
      enabled: enabledState === null ? false : enabledState,
    ...
    if (entry.enabled) {
      printActions("System actions:", getEnableActions(entry));
    }
  ...
  if (command === "edit") {
    ...
    if (!name) {
      throw new Error(
        "Usage: lanx edit <name> [--target <url> | --port <n>] [--enable | --disable]",
      );
    }
    const domain = normalizeName(name);
    assertOnlyFlags(args, ["--target", "--port", "--enable", "--disable"]);
    ...
    const nextEnabled = parseEnabledState(args);
    if (nextTarget !== null && nextPort !== null) {
      throw new Error("Provide only one of --target or --port");
    }
    if (nextTarget === null && nextPort === null && nextEnabled === null) {
      throw new Error("Provide --target, --port, --enable, or --disable");
    }
